...
const setRouteLayer = (routeLayer, mymap) => {
    return (newRouteLayer) => {
        if (routeLayer) {
            mymap.removeLayer(routeLayer);
        }
        newRouteLayer.addTo(mymap);
        mymap.fitBounds(newRouteLayer.getBounds());
        return newRouteLayer;
    };
};
...
const setHighlightedSegment = (highlightedSegment, mymap) => {
    return (newSegment) => {
        if (highlightedSegment) {
            mymap.removeLayer(highlightedSegment);
        }
        newSegment.addTo(mymap);
        mymap.fitBounds(newSegment.getBounds());
        return newSegment;
    };
};
